...
`)) {
    ...
    for (let assembly of originalStack.slice(error.toString().length + 1).split(/\n/).reverse()) {
      ...
      matches && (frame.function = (_d = (_c = (_b = matches[2]) == null ? void 0 : _b.trim()) == null ? void 0 : _c.replace(/^[^.]{1,2}(\.|$)/, "_$1")) != null ? _d : frame.function, frame.filename = ((_f = (_e = matches[4]) == null ? void 0 : _e.trim()) != null ? _f : frame.filename).replace(/^\.\/dist\//, "/github-copilot/dist/"), matches[5] && matches[5] !== ":0" && (frame.lineno = matches[5].slice(1)), matches[6] && matches[5] !== ":0" && (frame.colno = matches[6].slice(1)), frame.in_app = !/[[<:]|(?:^|\/)node_modules\//.test(frame.filename)), exceptionDetail.stacktrace.push(frame);
    }
  }
  return exceptionDetail;
},__name(buildExceptionDetail, "buildExceptionDetail");

,function buildContext(ctx, extraProperties) {
  var _a, _b;
  let epInfo = ctx.get(EditorAndPluginInfo),
    ...
    context = {
      ...
      "#architecture": nL.arch(),
      "#os_platform": nL.platform(),
      ...extraProperties
    };
...
,function buildPayload(ctx, redactedError) {
  var _a, _b, _c, _d;
  ...
  for (let [exception, detail] of exceptionsWithDetails) if (detail.stacktrace && detail.stacktrace.length > 0) {
    ...
    for (let frame of stacktrace) if ((_b = frame.filename) != null && _b.startsWith("/github-copilot/")) return payload;
  ...
  return payload.exception_detail.length > 0 && (payload.rollup_id = (0, tL.SHA256)(tL.enc.Utf16.parse(rollup.join(`
`))).toString()), payload;

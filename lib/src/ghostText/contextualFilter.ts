var _ContextualFilterManager = class _ContextualFilterManager {
  constructor() {
    this.previousLabel = 0, this.previousLabelTimestamp = Date.now() - 3600, this.probabilityAccept = 0;
  }
};

,__name(_ContextualFilterManager, "ContextualFilterManager");

,var ContextualFilterManager = _ContextualFilterManager;
...
,function contextualFilterScore(ctx, telemetryData, prompt) {
  ...
  let probabilityAccept = 0,
    features = new Array(221).fill(0);
  return features[0] = yt_1, features[1] = acw, features[2] = ln_dt_1, features[3] = ln_promptLastLineLength, features[4] = ln_promptLastLineRstripLength, features[5] = ln_documentLength, features[6] = ln_promptEndPos, features[7] = relativeEndPos, features[8 + languageIndex] = 1, features[29 + promptLastCharIndex] = 1, features[125 + promptLastRstripCharIndex] = 1, probabilityAccept = treeScore(features)[1], ctx.get(ContextualFilterManager).probabilityAccept = probabilityAccept, probabilityAccept;

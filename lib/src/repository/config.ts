...
,__name(_GitConfigData, "GitConfigData");

,var GitConfigData = _GitConfigData,
  _GitConfigLoader = class _GitConfigLoader {};

,__name(_GitConfigLoader, "GitConfigLoader");

,var GitConfigLoader = _GitConfigLoader,
  _GitCLIConfigLoader = class _GitCLIConfigLoader extends GitConfigLoader {
    runCommand(cwd, cmd, args) {
      return new Promise((resolve, reject) => {
        (0, sSe.execFile)(cmd, args, {
          cwd: cwd
        }, (err, stdout) => {
    ...
    async getConfig(ctx, baseFolder) {
      let fsPath = getFsPath(baseFolder);
      if (fsPath === void 0) return;
      let output = await this.tryRunCommand(ctx, fsPath, "git", ["-c", "safe.directory=*", "config", "--list", "--null", ...this.extraArgs()]);
...
,__name(_GitCLIConfigLoader, "GitCLIConfigLoader");

,var GitCLIConfigLoader = _GitCLIConfigLoader,
...
,__name(_GitFallbackConfigLoader, "GitFallbackConfigLoader");

,var GitFallbackConfigLoader = _GitFallbackConfigLoader;
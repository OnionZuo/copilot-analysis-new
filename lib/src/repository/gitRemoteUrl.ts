...
,var _GitRemoteUrl = class _GitRemoteUrl {
  constructor(url) {
    this.url = url;
  ...
  getUrlForApi() {
    ...
    if (this.isUrl() && !this.isInvalid()) return factory_exports.from({
      scheme: this.scheme,
  ...
  parseUrl() {
    let uri;
    try {
      uri = factory_exports.parse(this.url);
    } catch (e) {
      this._error = e;
      return;
    }
  ...
  tryParseSSHString() {
    ...
    if (match && ((0, cSe.platform)() !== "win32" || ((_c = (_b = (_a = match.groups) == null ? void 0 : _a.host) == null ? void 0 : _b.length) != null ? _c : 0) > 1)) {
...
,__name(_GitRemoteUrl, "GitRemoteUrl");

,var GitRemoteUrl = _GitRemoteUrl;
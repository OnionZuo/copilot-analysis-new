...
,__name(_GitConfigParser, "GitConfigParser");

,var GitConfigParser = _GitConfigParser,
  _GitParsingConfigLoader = class _GitParsingConfigLoader extends GitConfigLoader {
    ...
    async tryLoadConfig(ctx, configFile, warnIfNotExists) {
      try {
        ...
        (warnIfNotExists || !(e instanceof Error) || e.code !== "ENOENT") && logger.warn(ctx, `Failed to load git config from ${configFile.toString()}:`, e);
        return;
      }
    }
    async baseConfig(ctx, baseConfigFile) {
      let commonUri = await this.commondirConfigUri(ctx, baseConfigFile),
        xdgUri = joinPath(this.xdgConfigUri(), "git", "config"),
        userUri = joinPath(this.homeUri(), ".gitconfig");
    ...
    async commondirConfigUri(ctx, baseConfigFile) {
      ...
      let dir = dirname(baseConfigFile),
        commondirFile = joinPath(dir, "commondir");
      try {
    ...
    xdgConfigUri() {
      return process.env.XDG_CONFIG_HOME ? factory_exports.file(process.env.XDG_CONFIG_HOME) : joinPath(this.homeUri(), ".config");
    }
    homeUri() {
      return factory_exports.file((0, Vze.homedir)());
    }
  };

,__name(_GitParsingConfigLoader, "GitParsingConfigLoader");

,var GitParsingConfigLoader = _GitParsingConfigLoader;
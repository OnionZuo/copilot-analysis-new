...
,var parseBlockLogger = new Logger("parseBlock");

,function parsingBlockFinished(ctx, doc, position) {
  let prefix = doc.getText(LocationFactory.range(LocationFactory.position(0, 0), position)),
...
,async function getNodeStart(ctx, doc, position, completion) {
  let text = doc.getText(LocationFactory.range(LocationFactory.position(0, 0), position)) + completion,
...
`),
    ...
    lastLineOfPreview = previewText == null ? void 0 : previewText.split(`
`).pop(),
    startLine = 0;
  if (isContinuation && (lastLineOfPreview == null ? void 0 : lastLineOfPreview.trim()) != "" && completionLines[0].trim() !== "" && startLine++, !isContinuation && OfferNextLineCompletion && completionLines[0].trim() === "" && startLine++, isContinuation || startLine++, completionLines.length === startLine) return "continue";
  let breakIndentation = Math.max(contextIndentation.current, (_a = contextIndentation.next) != null ? _a : 0);

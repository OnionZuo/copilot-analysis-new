var cobol_exports = {};,__export(cobol_exports, {
  default: () => cobol
});,var lang,
  cobol,
  init_cobol = __esmMin(() => {
    ...
    lang = Object.freeze({
      ...
      patterns: [{
        ...
        match: "^\\s+(\\d+)\\s+([0-9a-zA-Z][a-zA-Z\\-0-9_]+)\\s+((?i:constant))"
      }, {
        captures: {
          1: {
            name: "constant.cobol"
        ...
        match: "(?<![-_])((?i:picture\\s+is|picture|pic\\s+is|pic)\\s+[-+sS\\*$09aAbBxXuUpPnNzZ/,.]*)\\((\\d*)\\)([vV][-+sS\\*$09aAbBxXuUpPnNzZ/,\\.]*)\\((\\d*)\\)[-|+]"
      }, {
        captures: {
          1: {
        ...
        match: "(?<![-_])((?i:picture\\s+is|picture|pic\\s+is|pic)\\s+[-+sS\\*$09aAbBxXuUpPnNzZ/,.]*)\\((\\d*)\\)([vV\\.][-+s\\*$09aAbBsSnNxXuUzZ/,]*[0-9\\.()])*"
      }, {
        match: "(?<![-_])(?i:picture\\s+is|picture|pic\\s+is|pic)\\s+[-+sS\\*$09aAbBsSnpPNxXuUzZ/,.]*\\(\\d*\\)[Vv\\.][-+s\\*0$9aAbBsSnNxpPxXuUzZ/,]*",
        name: "storage.type.picture7.cobol"
      }, {
        match: "(?<![-_])(?i:picture\\s+is|picture|pic\\s+is|pic)\\s+[-+sS\\*$09aAbBsSnpPNxXuUzZ/,.]*\\(\\d*\\)[-+s\\*0$9aAbBsSnNxpPxXuUzZ/,]*[Vv\\.][-+s\\*0$9aAbBsSnNxpPxXuUzZ/,]*",
        ...
        match: "(?<![-_])((?i:picture\\s+is|picture|pic\\s+is|pic)\\s+[-+sS\\*$09aAbBsSnpPNxuUXzZ/,.]*)\\((\\d*)\\)[-+s\\*0$9aAbBsSnNxpPxXuUzZ/,]*"
      }, {
        match: "(?<![-_])(?i:picture\\s+is|picture|pic\\s+is|pic)\\s+[-+sS\\*$09aAbBsSnpNNxXuUzZ/,.]*\\(\\d*\\)",
        name: "storage.type.picture4.cobol"
      }, {
        match: "(?<![-_])(?i:picture\\s+is|picture|pic\\s+is|pic)\\s+[sS]?[9aAbBsSnNxXuUzZ]*[Vv][9aAxbXuUzZ]*\\(\\d*\\)",
        ...
        match: "((?<![-_])(?i:binary|computational-4|comp-4|computational-5|comp-5))\\((\\d*)\\)"
      }, {
      ...
      repository: {
        ...
        "dli-options": {
          match: "(?<![\\-\\w])(?i:statusgroup|checkpoint|chkp|id|lockclass|segment|info|where|from|using|keyfeedback|feedbacklen|variable|first|last|current|seglength|offset|locked|movenext|getfirst|set|setcond|setzero|setparent|fieldlength|keys|maxlength|length\\d*|area\\d*|psc|pcs|pcb|sysserve|into)(?![\\-\\w])",
          name: "keyword.other.dli"
        },
        "number-complex-constant": {
          match: "(\\-|\\+)?(((\\d+(\\.\\d+))|(\\.\\d+))((e|E)(\\+|-)?\\d+)?)([LlFfUuDd]|UL|ul)?(?=\\s|\\.$|,|\\))",
          name: "constant.numeric.cobol"
        },

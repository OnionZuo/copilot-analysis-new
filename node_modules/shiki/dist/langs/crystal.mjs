var crystal_exports = {};,__export(crystal_exports, {
  default: () => crystal
});,var lang,
  crystal,
  init_crystal = __esmMin(() => {
    init_html();
    init_sql();
    init_css();
    init_c();
    ...
    lang = Object.freeze({
      ...
      patterns: [{
        ...
        match: "^\\s*(abstract)?\\s*(class|struct|union|annotation|enum)\\s+(([.A-Z_:\\x{80}-\\x{10FFFF}][.\\w:\\x{80}-\\x{10FFFF}]*(\\(([,\\s.a-zA-Z0-9_:\\x{80}-\\x{10FFFF}]+)\\))?(\\s*(<)\\s*[.:A-Z\\x{80}-\\x{10FFFF}][.:\\w\\x{80}-\\x{10FFFF}]*(\\(([.a-zA-Z0-9_:]+\\s,)\\))?)?)|((<<)\\s*[.A-Z0-9_:\\x{80}-\\x{10FFFF}]+))",
        ...
        match: "\\b\\d[0-9_]*\\.\\d[0-9_]*([eE][+-]?[0-9_]+)?(f32|f64)?\\b",
        ...
        match: "\\b(?!0\\d)\\d[0-9_]*([ui](8|16|32|64|128))?\\b",
    ...
    }), crystal = [...html, ...sql, ...css, ...c, ...javascript, ...shellscript, lang];
  });
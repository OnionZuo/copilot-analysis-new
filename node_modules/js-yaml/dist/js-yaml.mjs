function isNothing(subject) {
...
},__name(isNothing, "isNothing");,function isObject(subject) {
  return typeof subject == "object" && subject !== null;
},__name(isObject, "isObject");,function toArray(sequence) {
  return Array.isArray(sequence) ? sequence : isNothing(sequence) ? [] : [sequence];
},__name(toArray, "toArray");,function extend(target, source) {
  var index, length, key, sourceKeys;
  if (source) for (sourceKeys = Object.keys(source), index = 0, length = sourceKeys.length; index < length; index += 1) key = sourceKeys[index], target[key] = source[key];
  return target;
},__name(extend, "extend");,function repeat(string, count) {
  var result = "",
    cycle;
  for (cycle = 0; cycle < count; cycle += 1) result += string;
  return result;
},__name(repeat, "repeat");,function isNegativeZero(number) {
  return number === 0 && Number.NEGATIVE_INFINITY === 1 / number;
},__name(isNegativeZero, "isNegativeZero");,var isNothing_1 = isNothing,
  isObject_1 = isObject,
  toArray_1 = toArray,
  repeat_1 = repeat,
  ...
  return exception.mark ? (exception.mark.name && (where += 'in "' + exception.mark.name + '" '), where += "(" + (exception.mark.line + 1) + ":" + (exception.mark.column + 1) + ")", !compact && exception.mark.snippet && (where += `

` + exception.mark.snippet), message + " " + where) : message;
},__name(formatError, "formatError");,function YAMLException$1(reason, mark) {
...
},__name(YAMLException$1, "YAMLException$1");,YAMLException$1.prototype = Object.create(Error.prototype);,YAMLException$1.prototype.constructor = YAMLException$1;,YAMLException$1.prototype.toString = __name(function (compact) {
  return this.name + ": " + formatError(this, compact);
}, "toString");,var exception = YAMLException$1;,function getLine(buffer, lineStart, lineEnd, position, maxLineLength) {
  var head = "",
    tail = "",
...
},__name(getLine, "getLine");,function padStart(string, max) {
  return common.repeat(" ", max - string.length) + string;
},__name(padStart, "padStart");,function makeSnippet(mark, options) {
  ...
  for (var re = /\r?\n|\r|\0/g, lineStarts = [0], lineEnds = [], match, foundLineNo = -1; match = re.exec(mark.buffer);) lineEnds.push(match.index), lineStarts.push(match.index + match[0].length), mark.position <= match.index && foundLineNo < 0 && (foundLineNo = lineStarts.length - 2);
  ...
  for (i = 1; i <= options.linesBefore && !(foundLineNo - i < 0); i++) line = getLine(mark.buffer, lineStarts[foundLineNo - i], lineEnds[foundLineNo - i], mark.position - (lineStarts[foundLineNo] - lineStarts[foundLineNo - i]), maxLineLength), result = common.repeat(" ", options.indent) + padStart((mark.line - i + 1).toString(), lineNoLength) + " | " + line.str + `
` + result;
...
`;
...
},__name(makeSnippet, "makeSnippet");,var snippet = makeSnippet,
  TYPE_CONSTRUCTOR_OPTIONS = ["kind", "multi", "resolve", "construct", "instanceOf", "predicate", "represent", "representName", "defaultStyle", "styleAliases"],
  YAML_NODE_KINDS = ["scalar", "sequence", "mapping"];,function compileStyleAliases(map) {
  var result = {};
...
},__name(compileStyleAliases, "compileStyleAliases");,function Type$1(tag, options) {
  if (options = options || {}, Object.keys(options).forEach(function (name) {
    ...
    return data;
  }, this.instanceOf = options.instanceOf || null, this.predicate = options.predicate || null, this.represent = options.represent || null, this.representName = options.representName || null, this.defaultStyle = options.defaultStyle || null, this.multi = options.multi || !1, this.styleAliases = compileStyleAliases(options.styleAliases || null), YAML_NODE_KINDS.indexOf(this.kind) === -1) throw new exception('Unknown kind "' + this.kind + '" is specified for "' + tag + '" YAML type.');
},__name(Type$1, "Type$1");,var type = Type$1;,function compileList(schema, name) {
  var result = [];
  return schema[name].forEach(function (currentType) {
...
},__name(compileList, "compileList");,function compileMap() {
  var result = {
      scalar: {},
      sequence: {},
      mapping: {},
  ...
  function collectType(type) {
    type.multi ? (result.multi[type.kind].push(type), result.multi.fallback.push(type)) : result[type.kind][type.tag] = result.fallback[type.tag] = type;
  }
  for (__name(collectType, "collectType"), index = 0, length = arguments.length; index < length; index += 1) arguments[index].forEach(collectType);
  return result;
},__name(compileMap, "compileMap");,function Schema$1(definition) {
  return this.extend(definition);
},__name(Schema$1, "Schema$1");,Schema$1.prototype.extend = __name(function (definition) {
  var implicit = [],
    explicit = [];
  if (definition instanceof type) explicit.push(definition);else if (Array.isArray(definition)) explicit = explicit.concat(definition);else if (definition && (Array.isArray(definition.implicit) || Array.isArray(definition.explicit))) definition.implicit && (implicit = implicit.concat(definition.implicit)), definition.explicit && (explicit = explicit.concat(definition.explicit));else throw new exception("Schema.extend argument should be a Type, [ Type ], or a schema definition ({ implicit: [...], explicit: [...] })");
  ...
  return null;
},__name(constructYamlNull, "constructYamlNull");,function isNull(object) {
  return object === null;
},__name(isNull, "isNull");,var _null = new type("tag:yaml.org,2002:null", {
  kind: "scalar",
...
},__name(resolveYamlBoolean, "resolveYamlBoolean");,function constructYamlBoolean(data) {
  return data === "true" || data === "True" || data === "TRUE";
},__name(constructYamlBoolean, "constructYamlBoolean");,function isBoolean(object) {
  return Object.prototype.toString.call(object) === "[object Boolean]";
},__name(isBoolean, "isBoolean");,var bool = new type("tag:yaml.org,2002:bool", {
  kind: "scalar",
...
},__name(isHexCode, "isHexCode");,function isOctCode(c) {
  return 48 <= c && c <= 55;
},__name(isOctCode, "isOctCode");,function isDecCode(c) {
  return 48 <= c && c <= 57;
},__name(isDecCode, "isDecCode");,function resolveYamlInteger(data) {
  if (data === null) return !1;
  var max = data.length,
    index = 0,
    hasDigits = !1,
  ...
  return !(!hasDigits || ch === "_");
},__name(resolveYamlInteger, "resolveYamlInteger");,function constructYamlInteger(data) {
  var value = data,
    sign = 1,
    ch;
...
},__name(constructYamlInteger, "constructYamlInteger");,function isInteger(object) {
  return Object.prototype.toString.call(object) === "[object Number]" && object % 1 === 0 && !common.isNegativeZero(object);
},__name(isInteger, "isInteger");,var int = new type("tag:yaml.org,2002:int", {
    kind: "scalar",
  ...
  YAML_FLOAT_PATTERN = new RegExp("^(?:[-+]?(?:[0-9][0-9_]*)(?:\\.[0-9_]*)?(?:[eE][-+]?[0-9]+)?|\\.[0-9_]+(?:[eE][-+]?[0-9]+)?|[-+]?\\.(?:inf|Inf|INF)|\\.(?:nan|NaN|NAN))$");,function resolveYamlFloat(data) {
  return !(data === null || !YAML_FLOAT_PATTERN.test(data) || data[data.length - 1] === "_");
},__name(resolveYamlFloat, "resolveYamlFloat");,function constructYamlFloat(data) {
  var value, sign;
...
},__name(constructYamlFloat, "constructYamlFloat");,var SCIENTIFIC_WITHOUT_DOT = /^[-+]?[0-9]+e/;,function representYamlFloat(object, style) {
  var res;
...
},__name(representYamlFloat, "representYamlFloat");,function isFloat(object) {
  return Object.prototype.toString.call(object) === "[object Number]" && (object % 1 !== 0 || common.isNegativeZero(object));
},__name(isFloat, "isFloat");,var float = new type("tag:yaml.org,2002:float", {
    kind: "scalar",
  ...
  YAML_TIMESTAMP_REGEXP = new RegExp("^([0-9][0-9][0-9][0-9])-([0-9][0-9]?)-([0-9][0-9]?)(?:[Tt]|[ \\t]+)([0-9][0-9]?):([0-9][0-9]):([0-9][0-9])(?:\\.([0-9]*))?(?:[ \\t]*(Z|([-+])([0-9][0-9]?)(?::([0-9][0-9]))?))?$");,function resolveYamlTimestamp(data) {
  return data === null ? !1 : YAML_DATE_REGEXP.exec(data) !== null || YAML_TIMESTAMP_REGEXP.exec(data) !== null;
},__name(resolveYamlTimestamp, "resolveYamlTimestamp");,function constructYamlTimestamp(data) {
  var match,
    year,
    month,
    day,
...
},__name(constructYamlTimestamp, "constructYamlTimestamp");,function representYamlTimestamp(object) {
  return object.toISOString();
},__name(representYamlTimestamp, "representYamlTimestamp");,var timestamp = new type("tag:yaml.org,2002:timestamp", {
  kind: "scalar",
...
},__name(resolveYamlMerge, "resolveYamlMerge");,var merge = new type("tag:yaml.org,2002:merge", {
    kind: "scalar",
...
\r`;,function resolveYamlBinary(data) {
  ...
  var code,
    ...
    max = data.length,
    map = BASE64_MAP;
  for (idx = 0; idx < max; idx++) if (code = map.indexOf(data.charAt(idx)), !(code > 64)) {
    if (code < 0) return !1;
    bitlen += 6;
  }
  return bitlen % 8 === 0;
},__name(resolveYamlBinary, "resolveYamlBinary");,function constructYamlBinary(data) {
  var idx,
    tailbits,
    input = data.replace(/[\r\n=]/g, ""),
    max = input.length,
    ...
    bits = 0,
    result = [];
  for (idx = 0; idx < max; idx++) idx % 4 === 0 && idx && (result.push(bits >> 16 & 255), result.push(bits >> 8 & 255), result.push(bits & 255)), bits = bits << 6 | map.indexOf(input.charAt(idx));
...
},__name(constructYamlBinary, "constructYamlBinary");,function representYamlBinary(object) {
  var result = "",
    bits = 0,
    idx,
    tail,
    max = object.length,
    map = BASE64_MAP;
  for (idx = 0; idx < max; idx++) idx % 3 === 0 && idx && (result += map[bits >> 18 & 63], result += map[bits >> 12 & 63], result += map[bits >> 6 & 63], result += map[bits & 63]), bits = (bits << 8) + object[idx];
...
},__name(representYamlBinary, "representYamlBinary");,function isBinary(obj) {
  return Object.prototype.toString.call(obj) === "[object Uint8Array]";
},__name(isBinary, "isBinary");,var binary = new type("tag:yaml.org,2002:binary", {
    kind: "scalar",
  ...
  _toString$2 = Object.prototype.toString;,function resolveYamlOmap(data) {
  if (data === null) return !0;
  var objectKeys = [],
    index,
    length,
  ...
  for (index = 0, length = object.length; index < length; index += 1) {
    ...
    for (pairKey in pair) if (_hasOwnProperty$3.call(pair, pairKey)) if (!pairHasKey) pairHasKey = !0;else return !1;
    if (!pairHasKey) return !1;
    if (objectKeys.indexOf(pairKey) === -1) objectKeys.push(pairKey);else return !1;
  }
  return !0;
},__name(resolveYamlOmap, "resolveYamlOmap");,function constructYamlOmap(data) {
  return data !== null ? data : [];
},__name(constructYamlOmap, "constructYamlOmap");,var omap = new type("tag:yaml.org,2002:omap", {
    kind: "sequence",
    resolve: resolveYamlOmap,
    construct: constructYamlOmap
  }),
  _toString$1 = Object.prototype.toString;,function resolveYamlPairs(data) {
  if (data === null) return !0;
  var index,
    length,
    pair,
  ...
  return !0;
},__name(resolveYamlPairs, "resolveYamlPairs");,function constructYamlPairs(data) {
  if (data === null) return [];
  var index,
    length,
    pair,
    ...
    result,
    object = data;
  for (result = new Array(object.length), index = 0, length = object.length; index < length; index += 1) pair = object[index], keys = Object.keys(pair), result[index] = [keys[0], pair[keys[0]]];
  return result;
},__name(constructYamlPairs, "constructYamlPairs");,var pairs = new type("tag:yaml.org,2002:pairs", {
    kind: "sequence",
    resolve: resolveYamlPairs,
    construct: constructYamlPairs
  }),
  _hasOwnProperty$2 = Object.prototype.hasOwnProperty;,function resolveYamlSet(data) {
  if (data === null) return !0;
  var key,
    object = data;
  for (key in object) if (_hasOwnProperty$2.call(object, key) && object[key] !== null) return !1;
  return !0;
},__name(resolveYamlSet, "resolveYamlSet");,function constructYamlSet(data) {
  return data !== null ? data : {};
},__name(constructYamlSet, "constructYamlSet");,var set = new type("tag:yaml.org,2002:set", {
    kind: "mapping",
  ...
  PATTERN_TAG_URI = /^(?:!|[^,\[\]\{\}])(?:%[0-9a-f]{2}|[0-9a-z\-#;\/\?:@&=\+\$,_\.!~\*'\(\)\[\]])*$/i;,function _class(obj) {
  return Object.prototype.toString.call(obj);
},__name(_class, "_class");,function is_EOL(c) {
  return c === 10 || c === 13;
},__name(is_EOL, "is_EOL");,function is_WHITE_SPACE(c) {
  return c === 9 || c === 32;
},__name(is_WHITE_SPACE, "is_WHITE_SPACE");,function is_WS_OR_EOL(c) {
  return c === 9 || c === 32 || c === 10 || c === 13;
},__name(is_WS_OR_EOL, "is_WS_OR_EOL");,function is_FLOW_INDICATOR(c) {
  return c === 44 || c === 91 || c === 93 || c === 123 || c === 125;
},__name(is_FLOW_INDICATOR, "is_FLOW_INDICATOR");,function fromHexCode(c) {
  var lc;
  return 48 <= c && c <= 57 ? c - 48 : (lc = c | 32, 97 <= lc && lc <= 102 ? lc - 97 + 10 : -1);
},__name(fromHexCode, "fromHexCode");,function escapedHexLen(c) {
  return c === 120 ? 2 : c === 117 ? 4 : c === 85 ? 8 : 0;
},__name(escapedHexLen, "escapedHexLen");,function fromDecimalCode(c) {
  return 48 <= c && c <= 57 ? c - 48 : -1;
},__name(fromDecimalCode, "fromDecimalCode");,function simpleEscapeSequence(c) {
  return c === 48 ? "\0" : c === 97 ? "\x07" : c === 98 ? "\b" : c === 116 || c === 9 ? "	" : c === 110 ? `
` : c === 118 ? "\v" : c === 102 ? "\f" : c === 114 ? "\r" : c === 101 ? "\x1B" : c === 32 ? " " : c === 34 ? '"' : c === 47 ? "/" : c === 92 ? "\\" : c === 78 ? "\x85" : c === 95 ? "\xA0" : c === 76 ? "\u2028" : c === 80 ? "\u2029" : "";
},__name(simpleEscapeSequence, "simpleEscapeSequence");,function charFromCodepoint(c) {
  return c <= 65535 ? String.fromCharCode(c) : String.fromCharCode((c - 65536 >> 10) + 55296, (c - 65536 & 1023) + 56320);
},__name(charFromCodepoint, "charFromCodepoint");,var simpleEscapeCheck = new Array(256),
  simpleEscapeMap = new Array(256);,for (i = 0; i < 256; i++) simpleEscapeCheck[i] = simpleEscapeSequence(i) ? 1 : 0, simpleEscapeMap[i] = simpleEscapeSequence(i);,var i;,function State$1(input, options) {
...
},__name(State$1, "State$1");,function generateError(state, message) {
  var mark = {
    name: state.filename,
...
},__name(generateError, "generateError");,function throwError(state, message) {
  throw generateError(state, message);
},__name(throwError, "throwError");,function throwWarning(state, message) {
  state.onWarning && state.onWarning.call(null, generateError(state, message));
},__name(throwWarning, "throwWarning");,var directiveHandlers = {
  YAML: __name(function (state, name, args) {
  ...
  TAG: __name(function (state, name, args) {
    var handle, prefix;
  ...
  if (start < end) {
    if (_result = state.input.slice(start, end), checkJson) for (_position = 0, _length = _result.length; _position < _length; _position += 1) _character = _result.charCodeAt(_position), _character === 9 || 32 <= _character && _character <= 1114111 || throwError(state, "expected valid JSON character");else PATTERN_NON_PRINTABLE.test(_result) && throwError(state, "the stream contains non-printable characters");
    state.result += _result;
  }
},__name(captureSegment, "captureSegment");,function mergeMappings(state, destination, source, overridableKeys) {
  var sourceKeys, key, index, quantity;
  for (common.isObject(source) || throwError(state, "cannot merge mappings; the provided source object is unacceptable"), sourceKeys = Object.keys(source), index = 0, quantity = sourceKeys.length; index < quantity; index += 1) key = sourceKeys[index], _hasOwnProperty$1.call(destination, key) || (destination[key] = source[key], overridableKeys[key] = !0);
},__name(mergeMappings, "mergeMappings");,function storeMappingPair(state, _result, overridableKeys, keyTag, keyNode, valueNode, startLine, startLineStart, startPos) {
  var index, quantity;
  if (Array.isArray(keyNode)) for (keyNode = Array.prototype.slice.call(keyNode), index = 0, quantity = keyNode.length; index < quantity; index += 1) Array.isArray(keyNode[index]) && throwError(state, "nested arrays are not supported inside keys"), typeof keyNode == "object" && _class(keyNode[index]) === "[object Object]" && (keyNode[index] = "[object Object]");
  ...
  return _result;
},__name(storeMappingPair, "storeMappingPair");,function readLineBreak(state) {
  var ch;
...
},__name(readLineBreak, "readLineBreak");,function skipSeparationSpace(state, allowComments, checkIndent) {
  for (var lineBreaks = 0, ch = state.input.charCodeAt(state.position); ch !== 0;) {
    for (; is_WHITE_SPACE(ch);) ch === 9 && state.firstTabInLine === -1 && (state.firstTabInLine = state.position), ch = state.input.charCodeAt(++state.position);
    if (allowComments && ch === 35) do ch = state.input.charCodeAt(++state.position); while (ch !== 10 && ch !== 13 && ch !== 0);
    if (is_EOL(ch)) for (readLineBreak(state), ch = state.input.charCodeAt(state.position), lineBreaks++, state.lineIndent = 0; ch === 32;) state.lineIndent++, ch = state.input.charCodeAt(++state.position);else break;
  }
  return checkIndent !== -1 && lineBreaks !== 0 && state.lineIndent < checkIndent && throwWarning(state, "deficient indentation"), lineBreaks;
},__name(skipSeparationSpace, "skipSeparationSpace");,function testDocumentSeparator(state) {
...
},__name(testDocumentSeparator, "testDocumentSeparator");,function writeFoldedLines(state, count) {
  count === 1 ? state.result += " " : count > 1 && (state.result += common.repeat(`
`, count - 1));
},__name(writeFoldedLines, "writeFoldedLines");,function readPlainScalar(state, nodeIndent, withinFlowCollection) {
  var preceding,
    following,
    captureStart,
    captureEnd,
    ...
    ch;
  if (ch = state.input.charCodeAt(state.position), is_WS_OR_EOL(ch) || is_FLOW_INDICATOR(ch) || ch === 35 || ch === 38 || ch === 42 || ch === 33 || ch === 124 || ch === 62 || ch === 39 || ch === 34 || ch === 37 || ch === 64 || ch === 96 || (ch === 63 || ch === 45) && (following = state.input.charCodeAt(state.position + 1), is_WS_OR_EOL(following) || withinFlowCollection && is_FLOW_INDICATOR(following))) return !1;
  for (state.kind = "scalar", state.result = "", captureStart = captureEnd = state.position, hasPendingContent = !1; ch !== 0;) {
    if (ch === 58) {
      ...
      if (is_EOL(ch)) if (_line = state.line, _lineStart = state.lineStart, _lineIndent = state.lineIndent, skipSeparationSpace(state, !1, -1), state.lineIndent >= nodeIndent) {
...
},__name(readPlainScalar, "readPlainScalar");,function readSingleQuotedScalar(state, nodeIndent) {
  var ch, captureStart, captureEnd;
  if (ch = state.input.charCodeAt(state.position), ch !== 39) return !1;
  for (state.kind = "scalar", state.result = "", state.position++, captureStart = captureEnd = state.position; (ch = state.input.charCodeAt(state.position)) !== 0;) if (ch === 39) {
    if (captureSegment(state, captureStart, state.position, !0), ch = state.input.charCodeAt(++state.position), ch === 39) captureStart = state.position, state.position++, captureEnd = state.position;else return !0;
...
},__name(readSingleQuotedScalar, "readSingleQuotedScalar");,function readDoubleQuotedScalar(state, nodeIndent) {
  var captureStart, captureEnd, hexLength, hexResult, tmp, ch;
  if (ch = state.input.charCodeAt(state.position), ch !== 34) return !1;
  for (state.kind = "scalar", state.result = "", state.position++, captureStart = captureEnd = state.position; (ch = state.input.charCodeAt(state.position)) !== 0;) {
    if (ch === 34) return captureSegment(state, captureStart, state.position, !0), state.position++, !0;
    if (ch === 92) {
      if (captureSegment(state, captureStart, state.position, !0), ch = state.input.charCodeAt(++state.position), is_EOL(ch)) skipSeparationSpace(state, !1, nodeIndent);else if (ch < 256 && simpleEscapeCheck[ch]) state.result += simpleEscapeMap[ch], state.position++;else if ((tmp = escapedHexLen(ch)) > 0) {
...
},__name(readDoubleQuotedScalar, "readDoubleQuotedScalar");,function readFlowCollection(state, nodeIndent) {
  var readNext = !0,
    _line,
    _lineStart,
    _pos,
    ...
    valueNode,
    ch;
  if (ch = state.input.charCodeAt(state.position), ch === 91) terminator = 93, isMapping = !1, _result = [];else if (ch === 123) terminator = 125, isMapping = !0, _result = {};else return !1;
...
},__name(readFlowCollection, "readFlowCollection");,function readBlockScalar(state, nodeIndent) {
  var captureStart,
    folding,
    ...
    tmp,
    ch;
  if (ch = state.input.charCodeAt(state.position), ch === 124) folding = !1;else if (ch === 62) folding = !0;else return !1;
  for (state.kind = "scalar", state.result = ""; ch !== 0;) if (ch = state.input.charCodeAt(++state.position), ch === 43 || ch === 45) CHOMPING_CLIP === chomping ? chomping = ch === 43 ? CHOMPING_KEEP : CHOMPING_STRIP : throwError(state, "repeat of a chomping mode identifier");else if ((tmp = fromDecimalCode(ch)) >= 0) tmp === 0 ? throwError(state, "bad explicit indentation width of a block scalar; it cannot be less than one") : detectedIndent ? throwError(state, "repeat of an indentation width identifier") : (textIndent = nodeIndent + tmp - 1, detectedIndent = !0);else break;
  if (is_WHITE_SPACE(ch)) {
    do ch = state.input.charCodeAt(++state.position); while (is_WHITE_SPACE(ch));
    if (ch === 35) do ch = state.input.charCodeAt(++state.position); while (!is_EOL(ch) && ch !== 0);
  }
  for (; ch !== 0;) {
...
`, didReadContent ? 1 + emptyLines : emptyLines), didReadContent = !0, detectedIndent = !0, emptyLines = 0, captureStart = state.position; !is_EOL(ch) && ch !== 0;) ch = state.input.charCodeAt(++state.position);
  ...
  return !0;
},__name(readBlockScalar, "readBlockScalar");,function readBlockSequence(state, nodeIndent) {
  var _line,
    _tag = state.tag,
    _anchor = state.anchor,
    _result = [],
  ...
  for (state.anchor !== null && (state.anchorMap[state.anchor] = _result), ch = state.input.charCodeAt(state.position); ch !== 0 && (state.firstTabInLine !== -1 && (state.position = state.firstTabInLine, throwError(state, "tab characters must not be used in indentation")), !(ch !== 45 || (following = state.input.charCodeAt(state.position + 1), !is_WS_OR_EOL(following))));) {
    ...
    if (_line = state.line, composeNode(state, nodeIndent, CONTEXT_BLOCK_IN, !1, !0), _result.push(state.result), skipSeparationSpace(state, !0, -1), ch = state.input.charCodeAt(state.position), (state.line === _line || state.lineIndent > nodeIndent) && ch !== 0) throwError(state, "bad indentation of a sequence entry");else if (state.lineIndent < nodeIndent) break;
  }
  return detected ? (state.tag = _tag, state.anchor = _anchor, state.kind = "sequence", state.result = _result, !0) : !1;
},__name(readBlockSequence, "readBlockSequence");,function readBlockMapping(state, nodeIndent, flowIndent) {
  var following,
    allowCompact,
    _line,
    _keyLine,
  ...
  if (state.firstTabInLine !== -1) return !1;
  for (state.anchor !== null && (state.anchorMap[state.anchor] = _result), ch = state.input.charCodeAt(state.position); ch !== 0;) {
    if (!atExplicitKey && state.firstTabInLine !== -1 && (state.position = state.firstTabInLine, throwError(state, "tab characters must not be used in indentation")), following = state.input.charCodeAt(state.position + 1), _line = state.line, (ch === 63 || ch === 58) && is_WS_OR_EOL(following)) ch === 63 ? (atExplicitKey && (storeMappingPair(state, _result, overridableKeys, keyTag, keyNode, null, _keyLine, _keyLineStart, _keyPos), keyTag = keyNode = valueNode = null), detected = !0, atExplicitKey = !0, allowCompact = !0) : atExplicitKey ? (atExplicitKey = !1, allowCompact = !0) : throwError(state, "incomplete explicit mapping pair; a key node is missed; or followed by a non-tabulated empty line"), state.position += 1, ch = following;else {
      if (_keyLine = state.line, _keyLineStart = state.lineStart, _keyPos = state.position, !composeNode(state, flowIndent, CONTEXT_FLOW_OUT, !1, !0)) break;
      if (state.line === _line) {
        for (ch = state.input.charCodeAt(state.position); is_WHITE_SPACE(ch);) ch = state.input.charCodeAt(++state.position);
        if (ch === 58) ch = state.input.charCodeAt(++state.position), is_WS_OR_EOL(ch) || throwError(state, "a whitespace character is expected after the key-value separator within a block mapping"), atExplicitKey && (storeMappingPair(state, _result, overridableKeys, keyTag, keyNode, null, _keyLine, _keyLineStart, _keyPos), keyTag = keyNode = valueNode = null), detected = !0, atExplicitKey = !1, allowCompact = !1, keyTag = state.tag, keyNode = state.result;else if (detected) throwError(state, "can not read an implicit mapping pair; a colon is missed");else return state.tag = _tag, state.anchor = _anchor, !0;
      } else if (detected) throwError(state, "can not read a block mapping entry; a multiline key may not be an implicit key");else return state.tag = _tag, state.anchor = _anchor, !0;
    }
    if ((state.line === _line || state.lineIndent > nodeIndent) && (atExplicitKey && (_keyLine = state.line, _keyLineStart = state.lineStart, _keyPos = state.position), composeNode(state, nodeIndent, CONTEXT_BLOCK_OUT, !0, allowCompact) && (atExplicitKey ? keyNode = state.result : valueNode = state.result), atExplicitKey || (storeMappingPair(state, _result, overridableKeys, keyTag, keyNode, valueNode, _keyLine, _keyLineStart, _keyPos), keyTag = keyNode = valueNode = null), skipSeparationSpace(state, !0, -1), ch = state.input.charCodeAt(state.position)), (state.line === _line || state.lineIndent > nodeIndent) && ch !== 0) throwError(state, "bad indentation of a mapping entry");else if (state.lineIndent < nodeIndent) break;
  }
...
},__name(readBlockMapping, "readBlockMapping");,function readTagProperty(state) {
  var _position,
    isVerbatim = !1,
    isNamed = !1,
    tagHandle,
  ...
  if (state.tag !== null && throwError(state, "duplication of a tag property"), ch = state.input.charCodeAt(++state.position), ch === 60 ? (isVerbatim = !0, ch = state.input.charCodeAt(++state.position)) : ch === 33 ? (isNamed = !0, tagHandle = "!!", ch = state.input.charCodeAt(++state.position)) : tagHandle = "!", _position = state.position, isVerbatim) {
    do ch = state.input.charCodeAt(++state.position); while (ch !== 0 && ch !== 62);
...
},__name(readTagProperty, "readTagProperty");,function readAnchorProperty(state) {
  var _position, ch;
...
},__name(readAnchorProperty, "readAnchorProperty");,function readAlias(state) {
...
},__name(readAlias, "readAlias");,function composeNode(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
  ...
  if (allowBlockCollections && (allowBlockCollections = atNewLine || allowCompact), (indentStatus === 1 || CONTEXT_BLOCK_OUT === nodeContext) && (CONTEXT_FLOW_IN === nodeContext || CONTEXT_FLOW_OUT === nodeContext ? flowIndent = parentIndent : flowIndent = parentIndent + 1, blockIndent = state.position - state.lineStart, indentStatus === 1 ? allowBlockCollections && (readBlockSequence(state, blockIndent) || readBlockMapping(state, blockIndent, flowIndent)) || readFlowCollection(state, flowIndent) ? hasContent = !0 : (allowBlockScalars && readBlockScalar(state, flowIndent) || readSingleQuotedScalar(state, flowIndent) || readDoubleQuotedScalar(state, flowIndent) ? hasContent = !0 : readAlias(state) ? (hasContent = !0, (state.tag !== null || state.anchor !== null) && throwError(state, "alias node should not have any properties")) : readPlainScalar(state, flowIndent, CONTEXT_FLOW_IN === nodeContext) && (hasContent = !0, state.tag === null && (state.tag = "?")), state.anchor !== null && (state.anchorMap[state.anchor] = state.result)) : indentStatus === 0 && (hasContent = allowBlockCollections && readBlockSequence(state, blockIndent))), state.tag === null) state.anchor !== null && (state.anchorMap[state.anchor] = state.result);else if (state.tag === "?") {
    ...
    if (_hasOwnProperty$1.call(state.typeMap[state.kind || "fallback"], state.tag)) type = state.typeMap[state.kind || "fallback"][state.tag];else for (type = null, typeList = state.typeMap.multi[state.kind || "fallback"], typeIndex = 0, typeQuantity = typeList.length; typeIndex < typeQuantity; typeIndex += 1) if (state.tag.slice(0, typeList[typeIndex].tag.length) === typeList[typeIndex].tag) {
      type = typeList[typeIndex];
      break;
    }
    type || throwError(state, "unknown tag !<" + state.tag + ">"), state.result !== null && type.kind !== state.kind && throwError(state, "unacceptable node kind for !<" + state.tag + '> tag; it should be "' + type.kind + '", not "' + state.kind + '"'), type.resolve(state.result, state.tag) ? (state.result = type.construct(state.result, state.tag), state.anchor !== null && (state.anchorMap[state.anchor] = state.result)) : throwError(state, "cannot resolve a node with !<" + state.tag + "> explicit tag");
  }
  return state.listener !== null && state.listener("close", state), state.tag !== null || state.anchor !== null || hasContent;
},__name(composeNode, "composeNode");,function readDocument(state) {
  var documentStart = state.position,
    ...
    hasDirectives = !1,
    ch;
  for (state.version = null, state.checkLineBreaks = state.legacy, state.tagMap = Object.create(null), state.anchorMap = Object.create(null); (ch = state.input.charCodeAt(state.position)) !== 0 && (skipSeparationSpace(state, !0, -1), ch = state.input.charCodeAt(state.position), !(state.lineIndent > 0 || ch !== 37));) {
    ...
    for (directiveName = state.input.slice(_position, state.position), directiveArgs = [], directiveName.length < 1 && throwError(state, "directive name must not be less than one character in length"); ch !== 0;) {
      ...
      if (ch === 35) {
        do ch = state.input.charCodeAt(++state.position); while (ch !== 0 && !is_EOL(ch));
        break;
      }
      if (is_EOL(ch)) break;
...
},__name(readDocument, "readDocument");,function loadDocuments(input, options) {
...
`), input.charCodeAt(0) === 65279 && (input = input.slice(1)));
...
},__name(loadDocuments, "loadDocuments");,function loadAll$1(input, iterator, options) {
...
},__name(loadAll$1, "loadAll$1");,function load$1(input, options) {
  var documents = loadDocuments(input, options);
  if (documents.length !== 0) {
    if (documents.length === 1) return documents[0];
    throw new exception("expected a single document in the stream, but found more");
  }
},__name(load$1, "load$1");,var loadAll_1 = loadAll$1,
  load_1 = load$1,
  loader = {
    loadAll: loadAll_1,
    load: load_1
  ...
  ESCAPE_SEQUENCES = {};,ESCAPE_SEQUENCES[0] = "\\0";,ESCAPE_SEQUENCES[7] = "\\a";,ESCAPE_SEQUENCES[8] = "\\b";,ESCAPE_SEQUENCES[9] = "\\t";,ESCAPE_SEQUENCES[10] = "\\n";,ESCAPE_SEQUENCES[11] = "\\v";,ESCAPE_SEQUENCES[12] = "\\f";,ESCAPE_SEQUENCES[13] = "\\r";,ESCAPE_SEQUENCES[27] = "\\e";,ESCAPE_SEQUENCES[34] = '\\"';,ESCAPE_SEQUENCES[92] = "\\\\";,ESCAPE_SEQUENCES[133] = "\\N";,ESCAPE_SEQUENCES[160] = "\\_";,ESCAPE_SEQUENCES[8232] = "\\L";,ESCAPE_SEQUENCES[8233] = "\\P";,var DEPRECATED_BOOLEANS_SYNTAX = ["y", "Y", "yes", "Yes", "YES", "on", "On", "ON", "n", "N", "no", "No", "NO", "off", "Off", "OFF"],
  DEPRECATED_BASE60_SYNTAX = /^[-+]?[0-9_]+(?::[0-9_]+)+(?:\.[0-9_]*)?$/;,function compileStyleMap(schema, map) {
  var result, keys, index, length, tag, style, type;
  if (map === null) return {};
  for (result = {}, keys = Object.keys(map), index = 0, length = keys.length; index < length; index += 1) tag = keys[index], style = String(map[tag]), tag.slice(0, 2) === "!!" && (tag = "tag:yaml.org,2002:" + tag.slice(2)), type = schema.compiledTypeMap.fallback[tag], type && _hasOwnProperty.call(type.styleAliases, style) && (style = type.styleAliases[style]), result[tag] = style;
  return result;
},__name(compileStyleMap, "compileStyleMap");,function encodeHex(character) {
  var string, handle, length;
  if (string = character.toString(16).toUpperCase(), character <= 255) handle = "x", length = 2;else if (character <= 65535) handle = "u", length = 4;else if (character <= 4294967295) handle = "U", length = 8;else throw new exception("code point within a string may not be greater than 0xFFFFFFFF");
  return "\\" + handle + common.repeat("0", length - string.length) + string;
},__name(encodeHex, "encodeHex");,var QUOTING_TYPE_SINGLE = 1,
  QUOTING_TYPE_DOUBLE = 2;,function State(options) {
...
},__name(State, "State");,function indentString(string, spaces) {
  for (var ind = common.repeat(" ", spaces), position = 0, next = -1, result = "", line, length = string.length; position < length;) next = string.indexOf(`
...
` && (result += ind), result += line;
  return result;
},__name(indentString, "indentString");,function generateNextLine(state, level) {
  return `
` + common.repeat(" ", state.indent * level);
},__name(generateNextLine, "generateNextLine");,function testImplicitResolving(state, str) {
  var index, length, type;
  for (index = 0, length = state.implicitTypes.length; index < length; index += 1) if (type = state.implicitTypes[index], type.resolve(str)) return !0;
  return !1;
},__name(testImplicitResolving, "testImplicitResolving");,function isWhitespace(c) {
  return c === CHAR_SPACE || c === CHAR_TAB;
},__name(isWhitespace, "isWhitespace");,function isPrintable(c) {
  return 32 <= c && c <= 126 || 161 <= c && c <= 55295 && c !== 8232 && c !== 8233 || 57344 <= c && c <= 65533 && c !== CHAR_BOM || 65536 <= c && c <= 1114111;
},__name(isPrintable, "isPrintable");,function isNsCharOrWhitespace(c) {
  return isPrintable(c) && c !== CHAR_BOM && c !== CHAR_CARRIAGE_RETURN && c !== CHAR_LINE_FEED;
},__name(isNsCharOrWhitespace, "isNsCharOrWhitespace");,function isPlainSafe(c, prev, inblock) {
  var cIsNsCharOrWhitespace = isNsCharOrWhitespace(c),
    cIsNsChar = cIsNsCharOrWhitespace && !isWhitespace(c);
  return (inblock ? cIsNsCharOrWhitespace : cIsNsCharOrWhitespace && c !== CHAR_COMMA && c !== CHAR_LEFT_SQUARE_BRACKET && c !== CHAR_RIGHT_SQUARE_BRACKET && c !== CHAR_LEFT_CURLY_BRACKET && c !== CHAR_RIGHT_CURLY_BRACKET) && c !== CHAR_SHARP && !(prev === CHAR_COLON && !cIsNsChar) || isNsCharOrWhitespace(prev) && !isWhitespace(prev) && c === CHAR_SHARP || prev === CHAR_COLON && cIsNsChar;
},__name(isPlainSafe, "isPlainSafe");,function isPlainSafeFirst(c) {
  return isPrintable(c) && c !== CHAR_BOM && !isWhitespace(c) && c !== CHAR_MINUS && c !== CHAR_QUESTION && c !== CHAR_COLON && c !== CHAR_COMMA && c !== CHAR_LEFT_SQUARE_BRACKET && c !== CHAR_RIGHT_SQUARE_BRACKET && c !== CHAR_LEFT_CURLY_BRACKET && c !== CHAR_RIGHT_CURLY_BRACKET && c !== CHAR_SHARP && c !== CHAR_AMPERSAND && c !== CHAR_ASTERISK && c !== CHAR_EXCLAMATION && c !== CHAR_VERTICAL_LINE && c !== CHAR_EQUALS && c !== CHAR_GREATER_THAN && c !== CHAR_SINGLE_QUOTE && c !== CHAR_DOUBLE_QUOTE && c !== CHAR_PERCENT && c !== CHAR_COMMERCIAL_AT && c !== CHAR_GRAVE_ACCENT;
},__name(isPlainSafeFirst, "isPlainSafeFirst");,function isPlainSafeLast(c) {
  return !isWhitespace(c) && c !== CHAR_COLON;
},__name(isPlainSafeLast, "isPlainSafeLast");,function codePointAt(string, pos) {
...
},__name(codePointAt, "codePointAt");,function needIndentIndicator(string) {
  var leadingSpaceRe = /^\n* /;
  return leadingSpaceRe.test(string);
},__name(needIndentIndicator, "needIndentIndicator");,var STYLE_PLAIN = 1,
  STYLE_SINGLE = 2,
  STYLE_LITERAL = 3,
  STYLE_FOLDED = 4,
  STYLE_DOUBLE = 5;,function chooseScalarStyle(string, singleLineOnly, indentPerLevel, lineWidth, testAmbiguousType, quotingType, forceQuotes, inblock) {
  var i,
    char = 0,
    prevChar = null,
    hasLineBreak = !1,
    ...
    previousLineBreak = -1,
    plain = isPlainSafeFirst(codePointAt(string, 0)) && isPlainSafeLast(codePointAt(string, string.length - 1));
  if (singleLineOnly || forceQuotes) for (i = 0; i < string.length; char >= 65536 ? i += 2 : i++) {
    if (char = codePointAt(string, i), !isPrintable(char)) return STYLE_DOUBLE;
    plain = plain && isPlainSafe(char, prevChar, inblock), prevChar = char;
  } else {
    for (i = 0; i < string.length; char >= 65536 ? i += 2 : i++) {
      if (char = codePointAt(string, i), char === CHAR_LINE_FEED) hasLineBreak = !0, shouldTrackWidth && (hasFoldableLine = hasFoldableLine || i - previousLineBreak - 1 > lineWidth && string[previousLineBreak + 1] !== " ", previousLineBreak = i);else if (!isPrintable(char)) return STYLE_DOUBLE;
      plain = plain && isPlainSafe(char, prevChar, inblock), prevChar = char;
    }
...
},__name(chooseScalarStyle, "chooseScalarStyle");,function writeScalar(state, string, level, iskey, inblock) {
  state.dump = function () {
    if (string.length === 0) return state.quotingType === QUOTING_TYPE_DOUBLE ? '""' : "''";
    if (!state.noCompatMode && (DEPRECATED_BOOLEANS_SYNTAX.indexOf(string) !== -1 || DEPRECATED_BASE60_SYNTAX.test(string))) return state.quotingType === QUOTING_TYPE_DOUBLE ? '"' + string + '"' : "'" + string + "'";
    var indent = state.indent * Math.max(1, level),
      lineWidth = state.lineWidth === -1 ? -1 : Math.max(Math.min(state.lineWidth, 40), state.lineWidth - indent),
      singleLineOnly = iskey || state.flowLevel > -1 && level >= state.flowLevel;
    function testAmbiguity(string) {
      return testImplicitResolving(state, string);
    }
    switch (__name(testAmbiguity, "testAmbiguity"), chooseScalarStyle(string, singleLineOnly, state.indent, lineWidth, testAmbiguity, state.quotingType, state.forceQuotes && !iskey, inblock)) {
      case STYLE_PLAIN:
        return string;
      case STYLE_SINGLE:
...
},__name(writeScalar, "writeScalar");,function blockHeader(string, indentPerLevel) {
...
`;
},__name(blockHeader, "blockHeader");,function dropEndingNewline(string) {
  return string[string.length - 1] === `
` ? string.slice(0, -1) : string;
},__name(dropEndingNewline, "dropEndingNewline");,function foldString(string, width) {
  for (var lineRe = /(\n+)([^\n]*)/g, result = function () {
...
` : "") + foldLine(line, width), prevMoreIndented = moreIndented;
  ...
  return result;
},__name(foldString, "foldString");,function foldLine(line, width) {
...
` + line.slice(curr + 1) : result += line.slice(start), result.slice(1);
},__name(foldLine, "foldLine");,function escapeString(string) {
  for (var result = "", char = 0, escapeSeq, i = 0; i < string.length; char >= 65536 ? i += 2 : i++) char = codePointAt(string, i), escapeSeq = ESCAPE_SEQUENCES[char], !escapeSeq && isPrintable(char) ? (result += string[i], char >= 65536 && (result += string[i + 1])) : result += escapeSeq || encodeHex(char);
  return result;
},__name(escapeString, "escapeString");,function writeFlowSequence(state, level, object) {
  var _result = "",
    _tag = state.tag,
    index,
    length,
    value;
  for (index = 0, length = object.length; index < length; index += 1) value = object[index], state.replacer && (value = state.replacer.call(object, String(index), value)), (writeNode(state, level, value, !1, !1) || typeof value > "u" && writeNode(state, level, null, !1, !1)) && (_result !== "" && (_result += "," + (state.condenseFlow ? "" : " ")), _result += state.dump);
  state.tag = _tag, state.dump = "[" + _result + "]";
},__name(writeFlowSequence, "writeFlowSequence");,function writeBlockSequence(state, level, object, compact) {
  var _result = "",
    _tag = state.tag,
    index,
    length,
  ...
  state.tag = _tag, state.dump = _result || "[]";
},__name(writeBlockSequence, "writeBlockSequence");,function writeFlowMapping(state, level, object) {
  var _result = "",
    _tag = state.tag,
  ...
  state.tag = _tag, state.dump = "{" + _result + "}";
},__name(writeFlowMapping, "writeFlowMapping");,function writeBlockMapping(state, level, object, compact) {
  var _result = "",
    _tag = state.tag,
    objectKeyList = Object.keys(object),
    index,
    ...
    explicitPair,
    pairBuffer;
  if (state.sortKeys === !0) objectKeyList.sort();else if (typeof state.sortKeys == "function") objectKeyList.sort(state.sortKeys);else if (state.sortKeys) throw new exception("sortKeys must be a boolean or a function");
  ...
  state.tag = _tag, state.dump = _result || "{}";
},__name(writeBlockMapping, "writeBlockMapping");,function detectType(state, object, explicit) {
  var _result, typeList, index, length, type, style;
  for (typeList = explicit ? state.explicitTypes : state.implicitTypes, index = 0, length = typeList.length; index < length; index += 1) if (type = typeList[index], (type.instanceOf || type.predicate) && (!type.instanceOf || typeof object == "object" && object instanceof type.instanceOf) && (!type.predicate || type.predicate(object))) {
    if (explicit ? type.multi && type.representName ? state.tag = type.representName(object) : state.tag = type.tag : state.tag = "?", type.represent) {
      if (style = state.styleMap[type.tag] || type.defaultStyle, _toString.call(type.represent) === "[object Function]") _result = type.represent(object, style);else if (_hasOwnProperty.call(type.represent, style)) _result = type.represent[style](object, style);else throw new exception("!<" + type.tag + '> tag resolver accepts not "' + style + '" style');
      state.dump = _result;
    }
    return !0;
  }
  return !1;
},__name(detectType, "detectType");,function writeNode(state, level, object, block, compact, iskey, isblockseq) {
  ...
  return !0;
},__name(writeNode, "writeNode");,function getDuplicateReferences(object, state) {
  var objects = [],
    duplicatesIndexes = [],
    index,
    length;
  for (inspectNode(object, objects, duplicatesIndexes), index = 0, length = duplicatesIndexes.length; index < length; index += 1) state.duplicates.push(objects[duplicatesIndexes[index]]);
  state.usedDuplicates = new Array(length);
},__name(getDuplicateReferences, "getDuplicateReferences");,function inspectNode(object, objects, duplicatesIndexes) {
  var objectKeyList, index, length;
  if (object !== null && typeof object == "object") if (index = objects.indexOf(object), index !== -1) duplicatesIndexes.indexOf(index) === -1 && duplicatesIndexes.push(index);else if (objects.push(object), Array.isArray(object)) for (index = 0, length = object.length; index < length; index += 1) inspectNode(object[index], objects, duplicatesIndexes);else for (objectKeyList = Object.keys(object), index = 0, length = objectKeyList.length; index < length; index += 1) inspectNode(object[objectKeyList[index]], objects, duplicatesIndexes);
},__name(inspectNode, "inspectNode");,function dump$1(input, options) {
  options = options || {};
  var state = new State(options);
  state.noRefs || getDuplicateReferences(input, state);
  var value = input;
  return state.replacer && (value = state.replacer.call({
    "": value
  }, "", value)), writeNode(state, 0, value, !0, !0) ? state.dump + `
` : "";
},__name(dump$1, "dump$1");,var dump_1 = dump$1,
  dumper = {
    dump: dump_1
...
},__name(renamed, "renamed");,var load = loader.load,
  loadAll = loader.loadAll,
  dump = dumper.dump;,var safeLoad = renamed("safeLoad", "load"),

var languageMarkers = {
  ...
  mdLanguageIdToLanguageId = {};

,for (let [languageId, info] of Object.entries(languageMarkers)) if (info.markdownLanguageIds) for (let mdLanguageId of info.markdownLanguageIds) mdLanguageIdToLanguageId[mdLanguageId] = languageId;else mdLanguageIdToLanguageId[languageId] = languageId;

,var defaultCommentMarker = {
    start: "//",
    end: ""
  },
...
,function comment(text, languageId) {
  let markers = languageMarkers[languageId] ? languageMarkers[languageId].lineComment : defaultCommentMarker;
  if (markers) {

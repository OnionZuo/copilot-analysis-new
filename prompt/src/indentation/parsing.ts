...
,function groupBlocks(tree, isDelimiter = isBlank, label) {
  return rebuildTree(tree, __name(function (tree) {
    ...
    for (let i = 0; i < tree.subs.length; i++) {
      ...
      !subIsDelimiter && lastNodeWasDelimiter && (flushBlockIntoNewSubs(), nodesSinceLastFlush = []), lastNodeWasDelimiter = subIsDelimiter, nodesSinceLastFlush.push(sub), isBlank(sub) || (currentBlockIndentation = currentBlockIndentation != null ? currentBlockIndentation : sub.indentation);
    }
...
,function parseTree(source, languageId) {
  let raw = parseRaw(source),
    languageSpecificParser = LANGUAGE_SPECIFIC_PARSERS[languageId != null ? languageId : ""];
